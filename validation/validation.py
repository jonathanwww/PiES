...
class EquationTransformer(Transformer):
    ...
    Use by sending a lark.Tree to the validate method
        returns sets of the functions and variables in the equation
    """
    def __init__(self):
        super().__init__()
        self.valid = True
    ...
    def atom(self, items):
        for item in items:
            # If the item is a Token with type CNAME, add it to the set of variables
    ...
    def function_call(self, items):
        ...
        self.functions.add(str(function_name))
        return items
...
class ValidateUnits(Transformer):
    ...
    def function_call(self, function):
        ...
        if func_name in self.functions:
            ...
            return Variable(func_name, self.unit_registry[unit])
        else:
            return Variable(func_name, self.unit_registry.dimensionless)
